/* m7.c -- Choplifter! for the IBM PCjr, milestone M7.
 *
 * DESIGN.md section 13: combat.  M6 hostages plus tanks, jets, saucers,
 * bullets, collisions, house fires, explosion / sink, and the three-sortie
 * cycle.  FIRST SORTIE is HUD-font text for ~2 s at play start (full
 * sortie-banner art is M9).  Death plays an SN76496 noise rumble (the
 * rest of the effect table is M8).  No win-lose art (M9).
...
static unsigned char    snd_death_att = 16;     /* 0-15 playing, 16 idle */
...
/* SN76496 at 0C0h.  Port 61h bits 5-6 must be set or a real PCjr is silent
 * (jrpiano3; DOSBox ignores the gate).  Death-only; the rest of the table
 * is M8. */
...
static void snd_death_start(void)
{
    snd_gate_on();
    /* Periodic noise clocked off tone 2 + a low tone: crash rumble. */
    snd_psg(0xC0 | (unsigned char)(0x3A0 & 0x0F));
    snd_psg((unsigned char)((0x3A0 >> 4) & 0x3F));
    snd_psg(0xD0);
    snd_psg(0xE4);
    snd_psg(0xF0);
    snd_psg(0x80 | (unsigned char)(0x384 & 0x0F));
    snd_psg((unsigned char)((0x384 >> 4) & 0x3F));
    snd_psg(0x90);
    snd_death_att = 0;
}
...
static void snd_tick(void)
{
    ...
    if (a >= 15U)
        ...
        snd_death_att = (unsigned char)(a + 1U);
}
...
#define BANNER_Y        90U             /* sky; original banner world Y $6B */
#define BANNER_INK      15
#define BANNER_STR      "FIRST SORTIE"
#define BANNER_CHARS    12U
...
{
    const char *s;
    unsigned i, r, c, x0, x, wpx, xb, wb;
    const unsigned char *g;

    if (banner_left == 0U)
        return;
    s = BANNER_STR;
    x0 = (M8_WIDTH_PX - BANNER_CHARS * HUD_CELL_W) / 2U;
    for (i = 0; i < BANNER_CHARS; i++) {
        g = hud_glyph(s[i]);
        for (r = 0; r < HUD_GLYPH_H; r++) {
            for (c = 0; c < HUD_GLYPH_W; c++) {
                if ((g[r] & (unsigned char)(8U >> c)) == 0)
                    continue;
                x = x0 + i * HUD_CELL_W + c;
                plot_px(seg, x, BANNER_Y + r, BANNER_INK);
            }
        }
    }
    wpx = BANNER_CHARS * HUD_CELL_W;
    xb = x0 / 2U;
    wb = (wpx + 1U) / 2U;
    dirty_add(list, xb, BANNER_Y, wb, HUD_GLYPH_H);
}
...
static void sim_tick(void)
{
    ...
    if (total_rescues >= 64U || hostages_killed >= 64U)
        game_over = 1;
    scroll_terrain();
    if ((sim_frame & 3U) == 0U)
        spawn_hostages();
...
static void run_viewer(void)
{
    ...
    for (i = 0; i < limit; i++) {
        ...
        if (kbd_is_down(SCAN_ESC) || game_over)
            break;
    }
...
"the next sortie (three lives).  64 rescued or 64 killed ends the run.\n");
}
...
int main(int argc, char **argv)
{
    ...
    printf("Choplifter! PCjr -- M7 combat\n");
    printf("tanks, jets, saucers, fire, death/sortie  ground %s\n\n",
           GROUND_COLOUR_NAME);
    ...
    printf("PASS by eye: title then chopper on the base pad, facing right;\n"
           ...
           "to the door and R counts up.  Tanks roll left of the fence;\n"
           "your shots and their shells collide.  A crash or hit plays the\n"
           "explosion / sink cycle, then the next sortie (three).  No\n"
           "full-screen copy, no trail, no flicker.  Esc ends.\n");
    cleanup();
    return 0;
}
