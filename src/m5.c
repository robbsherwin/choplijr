/* m5.c -- Choplifter! for the IBM PCjr, milestone M5.
 *
 * DESIGN.md section 13: chopper physics, 11-step tilt, joystick.  M4's
 * world (camera lead, mountain parallax, BROWN ground, barracks/base/fence)
 * plus blit_rle chopper.  Fire is stubbed (M7).  Not hostages, not the game.
 *
 ...
 *   - In-flight debug HUD on rows 0-7 (F1 or ` toggles).  Not the M9 counters.
...
/* Debug HUD (rows 0-7).  Raw WASD bits so S flicker is visible; physics
 * uses the S/A/D break-latches below.  F1 or ` toggles. */
...
static void draw_debug_hud(unsigned seg)
{
    char buf[36];
    char ts[8], xs[8], ys[8];
    unsigned i;
    ...
    sprintf(buf, "W%u S%u A%u D%u T%s X%s Y%s A%u.%u/%u",
            (unsigned)hud_w, (unsigned)hud_s, (unsigned)hud_a, (unsigned)hud_d,
            ts, xs, ys, (unsigned)hud_accely,
...
static void handle_ground(void)
{
    ...
    bottom = (int)chop_y - (int)ground_bottom_table[idx];
    if (bottom < 0) {
        chop_y = ground_bottom_table[idx];
        goto detected;
    }
    if (bottom == 0)
        goto detected;
...
static void chopper_physics(void)
{
    ...
    if (accely < 8U) {
        ...
        int bottom = (int)chop_y - (int)ground_bottom_table[gidx];
...
"axes.  Esc ends play.  F1 or ` toggles the debug HUD.  Sim is 20 Hz;\n"
"the screen may flip faster.\n");
}
...
int main(int argc, char **argv)
{
    ...
    printf("Choplifter! PCjr -- M5 flight\n");
    printf("title, physics, 11-step tilt, joystick  ground %s\n\n",
           GROUND_COLOUR_NAME);
    ...
    printf("PASS by eye: title then chopper on the base pad, facing right;\n"
           ...
           "translate opposite ways; camera lead 80/240; mountains parallax;\n"
           "BROWN ground; debug HUD on rows 0-7 (F1 or ` toggles); Alt or\n"
           "button 1 rotates through the 11 tilts while airborne; no\n"
           "full-screen copy, no trail, no flicker.  Fire is a stub.  Esc\n"
           "ends.\n");
    cleanup();
    return 0;
}
